...
fn start_vm(list_c: Vec<String>, diff_list_all: HashMap<Atom, FileEvent>, new_mgr: &Mgr, old_mgr: &Mgr, depend: Depend, gray_mgr: MutexGuard<GrayMgr>) {
    let js = JS::new(0, Atom::from("hotfix start vm"), Arc::new(NativeObjsAuth::new(None, None)), None).unwrap();
...
fn write_depend_diff(diff: &HashMap<Atom, FileEvent>, mgr: &Mgr, path: PathBuf, depend: &Depend) -> Mgr{
    ...
    let js = JS::new(0, Atom::from("hotfix write depend diff vm"), Arc::new(NativeObjsAuth::new(None, None)), None).unwrap();
