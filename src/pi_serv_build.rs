...
fn call_1993779671(js: Arc<JS>, v: Vec<JSType>) -> Option<CallResult> {
    ...
    let mut result = ptr_jstype(js.get_objs(), js.clone(), ptr, 2627601653);
...
fn call_1347190475(js: Arc<JS>, v: Vec<JSType>) -> Option<CallResult> {
    ...
    let ptr = Box::into_raw(Box::new(result)) as usize;
    let mut result = ptr_jstype(js.get_objs(), js.clone(), ptr, 1721307497);
...
fn call_3344344275_async(js: Arc<JS>, v: Vec<JSType>) -> Option<CallResult> {
    ...
    let ptr = Box::into_raw(Box::new(result)) as usize;
    let mut result = ptr_jstype(js.get_objs(), js.clone(), ptr, 1496374710);
...
fn call_2175286088(js: Arc<JS>, v: Vec<JSType>) -> Option<CallResult> {
    ...
    let mut result = match result {
        Some(v) => {
            let mut v = js.new_str(String::from(v.as_str()));
    ...
    let call_back = move |r: (
        ...
        push_callback(
            ...
            Box::new(move |js: Arc<JS>| {
                ...
                let mut r_elem = r.1;
                let mut r_elem = match r_elem {
                    Ok(r) => {
                        let ptr = Box::into_raw(Box::new(r)) as usize;
                        let mut r = ptr_jstype(js.get_objs(), js.clone(), ptr, 606449873);
...
fn call_357009886(js: Arc<JS>, v: Vec<JSType>) -> Option<CallResult> {
    ...
    let ptr = Box::into_raw(Box::new(result)) as usize;
    let mut result = ptr_jstype(js.get_objs(), js.clone(), ptr, 1751456239);
...
fn call_3189305086(js: Arc<JS>, v: Vec<JSType>) -> Option<CallResult> {
    ...
    let ptr = Box::into_raw(Box::new(result)) as usize;
    let mut result = ptr_jstype(js.get_objs(), js.clone(), ptr, 3035778520);
...
fn call_3781439120(js: Arc<JS>, v: Vec<JSType>) -> Option<CallResult> {
    ...
    let ptr = Box::into_raw(Box::new(result)) as usize;
    let mut result = ptr_jstype(js.get_objs(), js.clone(), ptr, 589055833);
...
pub fn register(mgr: &BonMgr) {
    ...
    mgr.regist_struct_meta(
        StructMeta {
            name: String::from("pi_db::mgr::Event"),
            drop_fn: drop_3334364653,
        },
        3334364653,
    );
    mgr.regist_struct_meta(
    ...
    mgr.regist_struct_meta(
        StructMeta {
            name: String::from("net::data::Socket"),
            drop_fn: drop_3434008649,
        },
        3434008649,
    );
    mgr.regist_struct_meta(
        StructMeta {
            name: String::from("Arc<net::data::Socket>"),
            drop_fn: drop_589055833,
        },
        589055833,
    );
