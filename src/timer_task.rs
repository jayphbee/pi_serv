...
lazy_static! {
    ...
    // 任务分发通道
    static ref TASK_CHAN: (Sender<TimerTask>, Receiver<TimerTask>) = unbounded();
}
...
pub fn tick() {
    ...
    for (Item { elem, .. }, _) in expired {
        ...
        if let Some((vmf, dbmgr)) = fact {
            ...
            let _ = TASK_CHAN.0.send(TimerTask(vmf, real_args));
        }
    }
}
...
pub fn timer_task_loop() {
    ...
    let _ = thread::Builder::new()
        ...
        .spawn(move || loop {
            match TASK_CHAN.1.recv() {
                Ok(TimerTask(vmf, args)) => {
                    vmf.call(
                        None,
                ...
                Err(e) => {
                    error!("timer task channel error: {:?}", e);
                }
            }
        });
}
