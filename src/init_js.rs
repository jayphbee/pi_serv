...
pub fn init_js(dirs: &[String], file_list: Vec<FileDes>, root: String) {
    ...
    let js = JS::new(
        0,
        Atom::from("init js vm"),
        Arc::new(NativeObjsAuth::new(None, None)),
        None,
    )
    .unwrap();
    ...
    push_pre(&mut list);
    {
        ...
        SHELL_MANAGER
            .write()
            .unwrap()
            .add_natobj_env("_$db_mgr", mgr_ptr, 2976191628);
...
pub fn create_code_tab(mgr: &Mgr) {
    ...
    tr.alter(
        &ware,
        &tab,
        Some(Arc::new(TabMeta::new(EnumType::Str, EnumType::Bin))),
        Arc::new(|_r: SResult<()>| {}),
    );
